//Nombre: Christian Javier Ayala Guerra
//Carnet: AG19013
//Guia 61C: Programación Front-End Usando WebAPIs

//Creacion de cada una de las filas de la tabla
...
function listarProductos(productos) {
	//El getElementById va a acceder la informacion del HMTL con su respectivo Id
	var precio = document.getElementById("price");
	//Set Attribute --> Establece un valor a un elemento
	//Sintaxis --> Element.setAttribute(DOMstring con el Nombre del valor que va a cambiar, Un DOMString que contenga el valor que asignar al atributo);
	...
	tbody.innerHTML = "";
	var catcode;
	//Recorre con un for para obtener todas las categorias,y estas las asigna al HTML 
	//por medio del .innerHMTL y la asigna a la fila para que el tBody la capture en el
	//HTML
	...
	for (nfila = 0; nfila < num; nfila++) {
		...
		//Accion[nfila].innerHTML = "<button onclick='eliminarProductos(" + productos[nfila].id + ")'>Eliminar</button>";
	}

}

//Obtener los productos por medio de fetch, guardarlos como un Json y luego almacenarlo en la variable productos
function obtenerProductos() {
	 fetch('https://retoolapi.dev/5tQP5c/productos')
		.then(res => res.json())
		.then(data => {
			productos = data;
			productos.forEach(
...
function agregarProductos() {
	...
	//Capturar los valores del inputbox que se encuentran en el HTML
	...
	if(imagenUrl === "" || precioTxt === "" || titutoTxt === "" || categoriaTxt === "" || descripcionTxt === ""){
		alert("Debe de llenar todos los campos para agregar un producto! -- Se redirigirá a la pantalla de inicio");
	}else if(!precioExReg.test(precioTxt)){
		alert("El precio no es valido! -- Se redirigirá a la pantalla de inicio");
		return false;
	}else if(!urlExReg.test(imagenUrl)){
		alert("El URL proporcionado para la imagen no es valido! -- Se redirigirá a la pantalla de inicio");
		return false;
	}else{
		//Si todos los valores colocados en el input son correctos, se almacenaran en el JSON
		var nuevoProducto = {
			image: imagenUrl,
			price: precioTxt,
			title: titutoTxt,
		...
		//Extraigo todos los elementos y por medio del metodo Stringify los capturo como un string para mostrarlos.
		fetch('https://retoolapi.dev/5tQP5c/productos', {
		...
		alert("Se ha Agregado un producto correctamente");
	}
}
...
function eliminarProductos(iden) {
	fetch('https://retoolapi.dev/5tQP5c/productos/'+iden, { method: "DELETE" })
